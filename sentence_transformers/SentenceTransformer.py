...
class SentenceTransformer(nn.Sequential):
    ...
    def __init__(
        ...
        if model_name_or_path is not None and model_name_or_path != "":
            ...
            if model_path.startswith("http://") or model_path.startswith(
                ...
                if not os.path.exists(model_path) or not os.listdir(
                    model_path
                ):
                    if model_url[-1] == "/":
                    ...
                    model_path_tmp = (
                        model_path.rstrip("/").rstrip("\\") + "_part"
                    )
                    try:
                        zip_save_path = os.path.join(
                            model_path_tmp, "model.zip"
                        )
                        ...
                        if e.response.status_code == 404:
                            ...
                            logger.warning(
                                "Try to create Transformer Model {} with mean pooling".format(
                                    model_name_or_path
                                )
                            )
    ...
    def encode(
        ...
        if show_progress_bar is None:
            show_progress_bar = (
                logger.getLogger().getEffectiveLevel() == logger.INFO
                or logger.getLogger().getEffectiveLevel() == logger.DEBUG
            )
    ...
    def smart_batching_collate(self, batch, exists_weight=False):
        """
        ...
        if exists_weight:
            weights = torch.tensor(weights).to(self._target_device)
            return sentence_features, labels, weights
        else:
    ...
    def fit(
        ...
        for dataloader in dataloaders:
            if batch_weight:
                ...
                func = partial(
                    self.smart_batching_collate, exists_weight=True
                )
                dataloader.collate_fn = func
            else:
        ...
        for epoch in trange(epochs, desc="Epoch"):
            ...
            for _ in trange(
                steps_per_epoch, desc="Iteration", smoothing=0.05
            ):
                for train_idx in range(num_train_objectives):
                    ...
                    if use_amp:
                        ...
                        # TODO: reduction=None으로 loss_model을 만들면 여기서 List로 넘어옴. fit에서 weight를 받은 다음에 여기서 wieghted sum을 하도록 설정하면 될듯???
                        loss_value = loss_model(features, labels)
                        if batch_weight:
