...
class CrossEncoder:
    ...
    def smart_batching_collate(self, batch, exists_weight=False):
        ...
        if exists_weight:
            weights = torch.tensor(weights, dtype=torch.float).to(
                self._target_device
            )
            return tokenized, labels, weights
        else:
    ...
    def fit(
        ...
        if batch_weight:
            ...
            func = partial(self.smart_batching_collate, exists_weight=True)
            train_dataloader.collate_fn = func
        else:
        ...
        if loss_fct is None:
            # TODO: batch_weight가 있으면 reduction=None으로 만들기
            if batch_weight:
                loss_fct = (
        ...
        for epoch in trange(epochs, desc="Epoch"):
            ...
            for item in tqdm(
                train_dataloader, desc="Iteration", smoothing=0.05
            ):
                if not batch_weight:
                    features, labels = item
                else:
                    features, labels, weights = item
